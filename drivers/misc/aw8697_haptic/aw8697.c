...
static int aw8697_haptic_stop(struct aw8697 *aw8697)
{
    ...
    pr_debug("%s aw8697_haptic_stop_delay end\n", __func__);
...
static long aw8697_file_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case RICHTAP_STOP_MODE:
            dev_warn(aw8697->dev,"%s,RICHTAP_STOP_MODE  stop enter\n", __func__);
            ...
            aw8697_haptic_stop(aw8697);
            aw8697->haptic_rtp_mode = false;
            dev_warn(aw8697->dev,"%s,RICHTAP_STOP_MODE  stop end\n", __func__);
        ...
        struct device_attribute *attr, char *buf)
{
  ...
  //  unsigned char reg_val = 0;
    for(i = 0; i < AW8697_RTP_NUM; i ++) {
        len += snprintf(buf+len, PAGE_SIZE-len, "num: %u, serial:%u \n",i, aw8697->rtp_serial[i]);
    }
    return len;
}
...
#else
    len += snprintf(buf+len, PAGE_SIZE-len, "aw8697 lra f0 = %u\n", aw8697->f0);
#endif
    return len;
}
        ...
        char *buf)
{
    ...
    aw8697_haptic_get_f0(aw8697);
    mutex_unlock(&aw8697->lock);
    len += snprintf(buf+len, PAGE_SIZE-len, "aw8697 cali f0 = %u\n", aw8697->f0);
    return len;
}
        ...
        char *buf)
{
    ...
    len += snprintf(buf+len, PAGE_SIZE-len, "aw8697 cont f0 = %u\n", aw8697->cont_f0);
    return len;
}
...
#endif
    ssize_t len = 0;
    len += snprintf(buf+len, PAGE_SIZE-len, "aw8697 cont drv level = %u\n", aw8697->cont_drv_lvl);
    len += snprintf(buf+len, PAGE_SIZE-len, "aw8697 cont drv level overdrive= %u\n", aw8697->cont_drv_lvl_ov);
    return len;
}
...
#endif
    ssize_t len = 0;
    len += snprintf(buf+len, PAGE_SIZE-len, "aw8697 cont break num = %u\n", aw8697->cont_num_brk);
    return len;
}
        ...
        char *buf)
{
    ...
    len += snprintf(buf+len, PAGE_SIZE-len, "%u\n", aw8697->lra);
#else
    len += snprintf(buf+len, PAGE_SIZE-len, "r_lra=%umohm\n", aw8697->lra);
#endif
        ...
        char *buf)
{
    ...
    for(i=0; i<AW8697_TRIG_NUM; i++) {
        len += snprintf(buf+len, PAGE_SIZE-len,
            "trig%u: enable=%u, default_level=%u, dual_edge=%u, frist_seq=%u, second_seq=%u\n",
...
/* Add by zhengzhiwei@rm.bsp.sensor for audio_delay for debug interface */
...
static int aw8697_vibrator_init(struct aw8697 *aw8697)
{
    ...
    /* Modify by zhengzhiwei@rm.bsp.sensor for audio_delay for audio & vibrator sync */
...
#endif
    /* Add by zhengzhiwei@rm.bsp.sensor for audio_delay for audio & vibrator sync */
