...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = IMX615_SENSOR_ID_21881 - SENSOR_ID_OFFSET_21881,
...
static void read_EepromQSC(void)
{
    ...
    gImgEepromInfo_21881.camNormdata[1][0] = Eeprom_1ByteDataRead(0x00, 0xA8);
    gImgEepromInfo_21881.camNormdata[1][1] = Eeprom_1ByteDataRead(0x01, 0xA8);
    imgsensor_info.module_id = Eeprom_1ByteDataRead(0x00, 0xA8);
    Oplusimgsensor_Registdeviceinfo(gImgEepromInfo_21881.pCamModuleInfo[1].name,
                                    gImgEepromInfo_21881.pCamModuleInfo[1].version,
                                    imgsensor_info.module_id);
    for(i = 2; i < 8; i++) {
        gImgEepromInfo_21881.camNormdata[1][i] = Eeprom_1ByteDataRead(0x04+i, 0xA8);
    }
    for (i = 0; i < OPLUS_CAMERASN_LENS; i ++) {
        gImgEepromInfo_21881.camNormdata[1][8+i] = Eeprom_1ByteDataRead(0xB0+i, 0xA8);
    }
    gImgEepromInfo_21881.i4CurSensorIdx = 1;
    gImgEepromInfo_21881.i4CurSensorId = IMX615_SENSOR_ID_21881;
}
...
void imx615_fab_read()
{
    ...
    write_cmos_sensor_8(0x0A02, 0x7F);
    write_cmos_sensor_8(0x0A00, 0x01);
    msleep(1);

    reg_0xa01 = read_cmos_sensor_8(0x0A01);
    reg_0xa1f = read_cmos_sensor_8(0x0A1F);
    reg_0xa20 = read_cmos_sensor_8(0x0A20);

    LOG_INF("Read fab2 reg table [0xa01->0x%x, 0xa1f->0x%x, 0xa20->0x%x]", reg_0xa01, reg_0xa1f, reg_0xa20);
    if (reg_0xa01 == 0x01) {
        if (reg_0xa1f == 0xB4 && reg_0xa20 == 0x01) {
            msleep(2);
            write_cmos_sensor_8(0x0A00, 0x00);
            sensor_init();
            write_cmos_sensor_8(0x574B, 0x01);
            write_cmos_sensor_8(0x5765, 0x33);
        ...
        LOG_INF("Read 0xa01 vaule is 0x%x",reg_0xa01);
        LOG_INF("IMX615 read reg_0xa01 failed,just write init setting");
        sensor_init();
    }
}
/*************************************************************************
